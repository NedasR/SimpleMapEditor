...
public:
    ...
    public:
        ...
        sf::Color IndicatorButton(bool on, sf::Color Color = sf::Color::Black, sf::Color inicatorColor = sf::Color(20, 163, 57))
        {
            if (on)
            {
                return inicatorColor;
            }
            return Color;
        }
    };
    ...
    int main()
    {
        ...
        TilePicker picker;
        TileBoard board;
        sf::RenderWindow window(sf::VideoMode(WINDOW_WIDTH, WINDOW_HEIGHT), "coffie's game");
        while (window.isOpen())
        {
            sf::Event event;
            while (window.pollEvent(event))
            {
                ...
                if (picker.ButtonForPicker(event.mouseButton, 254, 0, 46, 30)) {
                    board.SaveMapEditor();
                    Time.restart();
                    Saving = true;
                }
                if (picker.ButtonForPicker(event.mouseButton,200, 0, 50, 30)) {
                    board.LoadMapEditor();
                    Time.restart();
                    Loading = true;
                }
            ...
            picker.PickerMText("Material Picker",window,20,6,4,sf::Color(57, 68, 150));
            picker.PickerMText("Load", window, 210,6,6, board.IndicatorButton(Loading));
            picker.PickerMText("Save", window, 260,6,6, board.IndicatorButton(Saving));
            window.display();
        }
        return 0;
    }